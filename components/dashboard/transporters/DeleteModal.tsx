...
import { useDeleteTransporterMutation } from '@/redux/api/transporterSlice';
...
export default function DeleteVehicleModal({ id, refresh }: Props) {
  const [deleteTransporter, { isLoading }] = useDeleteTransporterMutation();
  ...
  const handleDelete = () => {
    deleteTransporter(id)
      ...
      .catch((err) => {
        toast.success(t('delete_transporter_error'));
        setOpen(false);
      });
  };
